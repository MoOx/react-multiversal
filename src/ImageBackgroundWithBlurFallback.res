...
let make = (
  ...
  let (state, dispatch) = React.useReducer(
      ...
      | Loaded =>
        {
          ...
          start(
            state.animation->timing({
              open Value.Timing
              config(~toValue=1.->fromRawValue, ~duration=300., ~useNativeDriver=true, ())
            }),
            (),
          )
        }
      ...
      },
    {
      ...
      animation: Animated.Value.create(fallbackSource->Option.isSome ? 0. : 1.),
    },
  )
  let dimStyle = {
    open Style
  ...
  let fillImageStyle = {
    open Style
    arrayOption([Some(StyleSheet.absoluteFill), Some(dimStyle), styleImage])
  }
  <Animated.View
    style={
      open Style
          ...
          <Animated.View
            style={
              ...
              array([
                ...
                style(~opacity=state.animation->Animated.StyleProp.float, ()),
              ])
            }>
            <Image style=fillImageStyle resizeMode=#cover source onLoad={_ => dispatch(Loaded)} />
          </Animated.View>
        )
        ->Option.getWithDefault(React.null)}
